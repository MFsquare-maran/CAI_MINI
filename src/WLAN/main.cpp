...
void InitWiFi() {
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        digitalWrite(LED_ORANGE, HIGH); // LED aus Fehleranzeige
        system_shutdown();
    }
...
void write_logdata(float temperature, float pressure, float humidity, float gas_resistance, float battery_voltage)
{ 
    if (!SD.exists("/data.csv")) {
        logfile = SD.open("/data.csv", FILE_WRITE);
        logfile.println("Date,Temperature,Pressure,Humidity,Gas_Resistance,Battery_Voltage");
        logfile.close();
...
void timerTaskforshutdwon(void *parameter) {
    ...
    system_shutdown();   // 🔥 nach 30 Sekunden
    vTaskDelay(pdMS_TO_TICKS(1000)); // 1 Sekunde
    esp_restart(); // ESP neu starten

    while (1) {
        vTaskDelay(portMAX_DELAY); // Endlosschleife
    }
}
...
void setup() {
    ...
    if (bme.readSensor()) { // Sensorwerte lesen
        ...
        write_logdata(temperature, pressure, humidity, gas_resistance,battery_voltage); // Daten loggen
    } else {
    ...
    system_shutdown(); // System am Ende herunterfahren
}

// --- Loop-Funktion ---
void loop() {
    // Keine wiederkehrenden Aktionen, alles läuft im Setup und Tasks
}